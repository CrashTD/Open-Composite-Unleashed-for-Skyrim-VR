...
// IVRCompositor_028, as shipped in OpenVR SDK 2.5.1 (the version vendored by CommonLibVR 4.x).
// Valve never published a standalone 1.x header containing this version, so it lives in
// custom_interfaces like IVRSystem_023. Relative to IVRCompositor_027 the only change is the
// ADDITION of SubmitWithArrayIndex, at vtable slot 6 immediately after Submit (every other
// method is identical and in the same vtable order).

namespace vr
{
namespace IVRCompositor_028
{
...
public:
	...
	*/
	...
	/** Same as Submit, but supports passing an array index to submit from a texture array. */
