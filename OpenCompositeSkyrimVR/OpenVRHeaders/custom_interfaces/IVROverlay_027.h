...
// IVROverlay_027, as shipped in OpenVR SDK 2.5.1 (the version vendored by CommonLibVR 4.x).
// Valve never published a standalone 1.x header containing this version, so it lives in
// custom_interfaces like IVRSystem_023. Relative to IVROverlay_026 the only change is the
// REMOVAL of GetOverlayTransformOverlayRelative and SetOverlayTransformOverlayRelative
// (every other method is identical and in the same vtable order).

namespace vr
{
namespace IVROverlay_027
{
...
#pragma pack( push, 8 )
	...
	public:
		...
		* drawing the device. Overlays with this transform type cannot receive mouse events. */
		...
		// NOTE: IVROverlay_027 removed GetOverlayTransformOverlayRelative and
		// SetOverlayTransformOverlayRelative, which sat here in IVROverlay_026.
