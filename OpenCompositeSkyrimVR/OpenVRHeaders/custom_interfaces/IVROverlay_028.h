...
#include "generated/interfaces/version.h"
#include "generated/interfaces/public_vrtypes.h"
#include "generated/interfaces/vrtypes.h"
#include "generated/interfaces/vrannotation.h"

// IVROverlay_028, as shipped in OpenVR SDK 2.12.14 and retained by SDK 2.15.6.
// It preserves IVROverlay_027's vtable and inserts CreateSubviewOverlay immediately after
// CreateOverlay plus SetSubviewPosition immediately after SetOverlayTransformProjection.
// Keep this as a separate interface: aliasing 027 as 028 would shift every later vtable slot.

namespace vr
{
namespace IVROverlay_028
{
...
#pragma pack( push, 8 )
	...
	enum VROverlayFlags
	{
		...
		// If this is set, the overlay control bar will provide a close button.
		VROverlayFlags_EnableControlBarClose = 1 << 25,

		// Use a minimal control bar on the overlay.
		VROverlayFlags_MinimalControlBar = 1 << 26,

		// Apply click stabilization to laser interaction.
		VROverlayFlags_EnableClickStabilization = 1 << 27,

		// Permit mouse events for a secondary laser cursor.
	...
	public:
		...
		/** Creates a subview composited onto a parent overlay using a 2D transform. */
		...
		* drawing the device. Overlays with this transform type cannot receive mouse events. */
		...
		// IVROverlay_027 and _028 omit the two deprecated overlay-relative
		// transform methods that occupied these slots in IVROverlay_026.
		...
		* the texture and 1,1 in the lower right corner of the texture. */
		...
		virtual EVROverlayError SetOverlayTransformProjection( VROverlayHandle_t ulOverlayHandle,
			...
			const VROverlayProjection_t *pProjection, EVREye eEye ) = 0;

		/** Positions a subview in the parent overlay's pixel coordinate space. */
